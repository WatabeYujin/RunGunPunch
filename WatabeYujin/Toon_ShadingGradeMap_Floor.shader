Shader "UnityChanToonShader/Toon_ShadingGradeMap_Floor" {
    Properties {
...
//追記分
		_FloorSpeed("FloorSpeed",Float) = 0
...
#pragma target 3.0
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				fixed2 uv = IN.uv_BaseMap;
				float t = ((2 * _SinTime.w * _CosTime.w) + 1.0)*0.5;
				float e = tex2D(_BaseMap, IN.uv_BaseMap).a * t;
				uv.y += _FloorSpeed*10 * -_Time;
				o.Albedo = tex2D(_BaseMap, uv);
			}
